 ...
 *
 * {@link AbstractGameComponentManager}
 * @author hemantasapkota
 */
public abstract class AbstractGameComponentManager implements
		IGameComponentManager {
	 ...
	 * @param cam the cam
	 */
	public AbstractGameComponentManager(CGGameModel model, World world,
			Camera cam) {
		this.world = world;
		this.model = model;
		this.cam = cam;
	...
	protected void createJoints(CGShape shape) {
		...
		for (CGJoint joint : joints) {
			...
			if (jd != null) {
				...
				System.err
						.println("JointDef is not supposed to be null at this point.");
			}
		}
	...
	private JointDef initJoints(CGJoint joint, Body bodyA, Body bodyB) {
		...
		case DISTANCE:
			jd = ProtoBufTypeConversionUtil.asDistanceJointDef(bodyA, bodyB,
					joint.getDistanceJointDef());
			break;
		case FRICTION:
			jd = ProtoBufTypeConversionUtil.asFrictionJointDef(bodyA, bodyB,
					joint.getFrictionJointDef());
			break;
		case PRISMATIC:
			jd = ProtoBufTypeConversionUtil.asPrimasticJointDef(bodyA, bodyB,
					joint.getPrismaticJointDef());
			break;
		case PULLEY:
			jd = ProtoBufTypeConversionUtil.asPulleyJointDef(bodyA, bodyB,
					joint.getPulleyJointDef());
			break;
		case REVOLUTE:
			jd = ProtoBufTypeConversionUtil.asRevoluteJoint(bodyA, bodyB,
					joint.getRevoluteJointDef());
			break;
		case WELD:
			jd = ProtoBufTypeConversionUtil.asWeldJointDef(bodyA, bodyB,
					joint.getWeldJointDef());
			break;
		case ROPE:
			break;
		case GEAR:
	...
	protected Body createBody(CGShape shape, CGEntity entity) {
		BodyDef bodyDef = ProtoBufTypeConversionUtil.asBodyDef(shape
				.getBodyDef());
		FixtureDef fixDef = ProtoBufTypeConversionUtil.asFixtureDef(shape
				.getFixtureDef());
		...
		case ENTITY_SHAPE:
			bodyDef.position.set(position);
			b = world().createBody(bodyDef);
			createEntityCollisionShape(shape, entity, bodyDef, fixDef, b,
					position);
			break;
